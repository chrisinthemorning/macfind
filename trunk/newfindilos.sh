...
#
scriptversion="1.1"
# Added UUID fields for warranty support
...
#
# Tested with: Nmap 4.20, curl 7.17.1, RHEL4 # # Note: Discovery of an iLO is dependent upon the Virtual Media port
...
#
# 	run like this:	sudo bash newfindilos 10.60.10.0/24 > my.ilos
...
#
# scan a range of IP addresses looking for an # open tcp port 17988 (the iLO virtual media port) #
...
while read iloip <&3 ; do
  ...
  parseiloxml UUID; ilouuid=$parsedstring

  ilopid=$(echo $ilouuid  | sed "s/$sernum//")
...
#  printf "%-15s %-6s %-8s %-12s %s\n" $iloip "$ilohwver" "$ilofirmware" "$sernum" "$servermodel"
  echo "\"$iloip\",\"$ilohwver\",\"$ilofirmware\",\"$sernum\",\"$servermodel\",\"$ilopid\""
